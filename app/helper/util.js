...
module.exports = {
  setSchedule(date, callback) {
    schedule.scheduleJob(date, callback);
  },
  async getOne() { // 获取每日一句
    let res = await this.app.curl(this.ctx.config.ONE, {
      method: 'GET',
      dataType:'json',
    });
  ...
  async getWeather() { // 获取墨迹天气
    let url = this.ctx.config.MOJI_HOST + this.ctx.config.CITY + '/' + this.ctx.config.LOCATION
  ...
  contentDistinguish(contact, keywordArray) {
    ...
    if (keywordArray[2] === "每天") { // 判断是否属于循环任务
      ...
      scheduleObj.time = "0 " + array[1] + ' ' + array[0] + ' * * *';
  ...
  async getReply(word) { // 天行聊天机器人
    let url = this.ctx.config.AIBOTAPI;
    let res = await this.app.curl(url, {
      method: 'GET',
      dataType: 'json',
      data: {
        key: this.ctx.config.APIKEY,
        question: word,
      }
    });
