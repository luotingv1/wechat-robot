  ...
  async willReady() {
    ...
    const initDay = async () => {
      ...
      const contact = await bot.Contact.find({
        ...
        name: configs.NICKNAME,
      }); // 获取你要发送的联系人
      const one = await ctx.helper.util.getOne(); // 获取每日一句
      const weather = await ctx.helper.util.getWeather(); // 获取天气信息
      const today = await ctx.helper.util.formatDate(new Date()); // 获取今天的日期
      const memorialDay = ctx.helper.util.getDay(configs.MEMORIAL_DAY); // 获取纪念日天数
      const str = today + '<br>我们在一起的第' + memorialDay + '天<br><br>元气满满的一天开始啦,要开心噢^_^<br><br>今日天气<br>' + weather.weatherTips + '<br>' + weather.todayWeather + '<br>每日一句:<br>' + one + '<br><br>' + '————————最爱你的我'
      try {
        logMsg = str;
    ...
    const initSchedule = async (list) => {
      try {
        for (let item of list) {
          ...
          ctx.helper.util.setSchedule(time, async () => {
        ...
        // 登陆后初始化微信每日说
        ctx.helper.util.setSchedule(configs.SENDDATE, async () => {
    ...
    const onLogin = (user) => {
      console.log(`贴心助理${user}登录了`)
       app.curl(host + '/getScheduleList', {method: 'GET',dataType:'json'})
      .then(({data}) => {
    ...
    const onMessage = async (msg) => {
      ...
      const room = msg.room()
      const meiri = await bot.Room.find({
        topic: '微信每日说'
      })
      if (msg.self()) return
      if (room) {
        ...
        if (content.indexOf('开启了朋友验证') > -1 || contact.name() === '朋友推荐消息') { // 防止重复发送消息
          ...
          await delay(500)
          contact.say('1、回复关键词“加群”<br>2、或回复“提醒 我 18:30 下班回家”，创建你的专属提醒')
          ...
          if (keywordArray.length > 3) {
            let scheduleObj = ctx.helper.util.contentDistinguish(contact, keywordArray)
            if (keywordArray[2] === '每天') {
              ...
              console.log('日期格式', scheduleObj.time, '结果', ctx.helper.util.isRealDate(scheduleObj.time))
              let isTime = ctx.helper.util.isRealDate(scheduleObj.time)
              if (isTime) {
                addSchedule(scheduleObj)
                await delay(500)
                contact.say('小助手已经把你的提醒牢记在小本本上了')
          ...
          if (configs.AUTOREPLY) {
            let reply = await ctx.helper.util.getReply(content)
    ...
    const addSchedule = (obj) => {
      app.curl(host + '/addSchedule',{
        data:obj,
        method: 'POST',
        dataType:'json',
      }).then(async ({data}) => {
         data=data.data;
        let nickName = data.subscribe
        let time = data.time
        let Rule1 = data.isLoop ? time : new Date(time)
        let content = data.content
        let contact = await bot.Contact.find({
          name: nickName
        })
        ctx.helper.util.setSchedule(Rule1, async () => {
          console.log('你的专属提醒开启啦！')
          await delay(10000)
    ...
    function roomJoin(room, inviteeList, inviter) {
      ...
      room.topic().then(function(res) {
        const roomNameReg = eval(configs.ROOMNAME)
